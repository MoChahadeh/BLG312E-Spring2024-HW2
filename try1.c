...
void *MyMalloc(int size, int strategy) {
    ...
    size_t rounded_size = roundUpToPageSize(size + sizeof(BlockHeader));
    ...
    while (curr_block != NULL) {
        if (curr_block->size >= rounded_size) {
    ...
    // If the best fit block is larger than required size + minimum block header size
    // Split the block and allocate the requested size
    if (best_fit->size >= rounded_size + sizeof(BlockHeader)) {
        BlockHeader *new_block = (BlockHeader *)((char *)best_fit + rounded_size);
        new_block->size = best_fit->size - rounded_size;
        new_block->next = best_fit->next;
        best_fit->size = rounded_size;
...
int MyFree(void *ptr) {
    ...
    while (curr_block != NULL) {
        if ((char *)curr_block + curr_block->size == (char *)curr_block->next) {
