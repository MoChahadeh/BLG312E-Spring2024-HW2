...
void *MyMalloc(int size, int strategy) {
    if (size <= 0 || heap_start == NULL) {
        return NULL; // Invalid request size or uninitialized heap
    }
    ...
    while (current != NULL) {
        if (current->size >= rounded_size) {
            ...
            if (current->size >= rounded_size + sizeof(BlockHeader)) {
                // Split the block
                BlockHeader *new_block = (BlockHeader *)((char *)current + rounded_size);
                new_block->size = current->size - rounded_size;
            ...
            return (void *)(current + 1); // Return the memory excluding the header
        }
